      ...
      <table class="flex w-full h-full p-2">
        <tbody class="w-full">
          <tr class="flex w-full border-b" :style="{ borderColor: theme.separatorLineColor }"
            v-for="sim_display_item in sim_data_display" :key="sim_display_item.key">
    ...
    <SimPanel title="Simulation Controls 3" status="Status" :active=false class="panel-3">
      <div class="w-full h-full grid grid-cols-3 gap-1">
        <ButtonSwitch v-if="sim_module_loaded" v-for="(input, i) in simulationProps.Simulation" :key="i"
          ...
          :inputChange="input.setterFunc" class="border border-slate-600" :class="input.inputValue ? 'col-span-3' : ''">
        </ButtonSwitch>
      </div>
    </SimPanel>
    ...
    <SimPanel title="Simulation Control 6" status="Status" class="panel-6">
      <div class="w-full max-h-full grid gap-1">
        <template v-if="sim_module_loaded"
          ...
          <!-- <h3>{{ parentKey }}</h3> -->
          <button-switch class="border border-slate-600" v-for="(input, i) in simulationProps[parentKey]" :key="i"
            :buttonLabel="input.title" :textInput="input.inputValue" :inputChange="input.setterFunc"
            :inputMin="input.min" :inputMax="input.max" :inputStep="input.step"
            :class="!input.inputValue ? '' : ''"></button-switch>
        </template>
      </div>
    </SimPanel>
...
const sim_data_display = [
  ...
  { key: 'api_atmosphere_sea_level_temperature', label: 'Sea Level Temperature' },
...
onMounted(async () => {
  ...
  initializeModule({
    ...
    autopilotProps = computed(() => getAutopilotProperties(FlightSimModule, sim_data))
    simulationProps = computed(() => getSimulationParameters(FlightSimModule, sim_data))
    sim_module_loaded.value = true;

    // key presses are handled inside the canvas only
    window.removeEventListener('keydown', FlightSimModule.GLFW.onKeydown, true)
