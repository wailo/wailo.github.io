...
async function fetchAssetIdByName(name: string): Promise<{ id: number; digest: string }> {
...
function downloadAssetById(assetId: number): Promise<import('http').IncomingMessage> {
        ...
        .get(
          ...
          (res) => {
            ...
            if (statusCode !== 200) {
              ...
              res.on('end', () => {
                reject(
                  new Error(`Download failed (${statusCode}): ${errorBody || res.statusMessage}`),
                )
              })
              return
            }
...
async function run() {
  ...
  for (const asset of ASSETS) {
    ...
    const { id: assetId, digest: assetHash } = await fetchAssetIdByName(asset.name)
    ...
    // Verify Hash
    const actualHash = createHash('sha256').update(readFileSync(tempPath)).digest('hex')
