...
export async function main(context: ScriptContext) {
  ...
  const repositionPromise = repositionWithAutopilot(
    ...
    targetHeading,
    10000,
    () => flightModel.set_empty_mass(430), // arbitrary mass adjustment used to trim the aircraft close to zero pitch
  ).then(async () => {
    ////////////////////////////////////////////////////////////
  ...
  await waitForCondition(
    () => flightModel.speed_indicated_knots >= acceleratedSpeed - 0.1,
  );
  ...
  await waitForCondition(
    () => flightModel.speed_indicated_knots <= targetSpeed + 0.1,
  );
  await waitFor(4000);
