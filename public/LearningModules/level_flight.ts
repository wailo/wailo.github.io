...
export async function main(context: ScriptContext) {
  ...
  const repositionPromise = repositionWithAutopilot(
    ...
    targetHeading,
    10000,
    () => flightModel.set_empty_mass(396), // arbitrary mass adjustment used to trim the aircraft close to zero pitch
  ).then(async () => {
    ////////////////////////////////////////////////////////////
    // AUTOPILOT CONFIGURATION
    ////////////////////////////////////////////////////////////
    flightModel.set_freeze_fuel(true) // To freeze the weight
    ...
    flightModel.set_autopilot_heading_target(targetHeading)
  ...
  plotView([simProps.speed_indicated_knots, simProps.autopilot_speed_indicated_target], true)

  plotView([simProps.lift, simProps.weight], true)

  plotView([simProps.vertical_speed_ftmin, simProps.autopilot_vertical_speed_target], true)
  ...
  flightModel.set_autopilot_speed_indicated_target(acceleratedSpeed)
  ...
  await waitForCondition(() => flightModel.speed_indicated_knots >= acceleratedSpeed - 0.1)
  ...
  await waitForCondition(() => flightModel.speed_indicated_knots <= targetSpeed + 0.1)
  await waitFor(4000)
