...
export async function main(context: ScriptContext) {
  ...
  const success = await waitForCondition(
    () => {
      ...
      altitudeWithinLimits =
        Math.abs(currentAltitude - initialAltitude) <= maxAltitudeDeviation_ft;
      ...
      const headingChangedEnough =
        Math.abs(totalHeadingChange_deg) >= requiredHeadingChange_deg;
  ...
  if (success) {
    if (!altitudeWithinLimits) {
      ...
      // Scores
      const averageTurnRate =
        metrics.reduce((sum, m) => sum + m.turnRate, 0) / metrics.length;
