...
export async function main(context: ScriptContext) {
  ...
  const flapStages = [
    {
      angle: simControls.B747FlapSelector.ZERO,
      label: "Flaps Up (Clean)",
      pause: true,
    },
    {
      angle: simControls.B747FlapSelector.TEN,
      label: "Flaps 10°",
      pause: true,
    },
    {
      angle: simControls.B747FlapSelector.TWENTFIVE,
      label: "Flaps 20°",
      pause: true,
    },
    {
      angle: simControls.B747FlapSelector.THIRTY,
      label: "Flaps Full",
      pause: true,
    },
  ];
  ...
  notifyUser(
    "📘 Flap Effect",
    "Observe how changing flap settings affects airspeed, lift, and drag.",
  );
  await waitFor(4000);
